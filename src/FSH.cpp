...
int main(int argc, char* argv[]) {
	...
	if(argc < 2) {
		cerr<<"Please enter an input filename single-end: -si <AbsPathFile>"<<endl<<flush;
		cerr<<"Or Please enter an input filenames paired-end: -pi <AbsPathFile1> <AbsPathFile2>"<<endl<<flush;
		cerr<<"You can enter valid path for output directory with: -dirO"<<endl<<flush;
		cerr<<"You can enter a spaced seeds path as -q <AbsPathFile>. Every file's line must contain a spaced seeds. Enter q-mer with 1 at begin and end of the string"<<endl<<flush;
		return 0;
	}
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i], "-dirO") == 0)
		{
			...
			if(string(argv[i]) == "")
			{
				cerr<<endl<<"Enter valid path for output directory with: -dirO"<<flush;
				return 0;
			}
			else
				dir_output = argv[i];
